...
class ReportController extends Controller
{
    ...
    public function getStockBySellingPrice(Request $request)
    {
        ...
        $opening_stock_by_sp = $this->transactionUtil->getOpeningClosingStock($business_id, $day_before_start_date, $location_id, true, true, [], $end_date);

        $closing_stock_by_sp = $this->transactionUtil->getOpeningClosingStock( $business_id, $end_date, $location_id, false, true, [], null);
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
           ...
           ->addColumn('action', function ($row) {
                return '<a class="btn btn-xs btn-info text-nowrap" href="'.action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], [$row->product_id]).
            ...
            $raw_columns  = ['unit_price', 'total_transfered', 'total_sold',
                    'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price', 'potential_profit','action'];
    ...
    public function productSellReportBy(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $base_results = $query->get();

            $combo_query = TransactionSellLine::join('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
                ->where('t.business_id', $business_id)
                ->where('t.type', 'sell')
                ->where('t.status', 'final')
                ->where('transaction_sell_lines.children_type', 'combo')
                ->whereNotNull('transaction_sell_lines.combo_modifier_ids')
                ->where('transaction_sell_lines.combo_modifier_ids', '!=', '');

            if (!empty($start_date) && !empty($end_date)) {
                $combo_query->whereDate('t.transaction_date', '>=', $start_date)->whereDate('t.transaction_date', '<=', $end_date);
            }
            if ($permitted_locations != 'all') {
                $combo_query->whereIn('t.location_id', $permitted_locations);
            }
            if (!empty($location_id)) {
                $combo_query->where('t.location_id', $location_id);
            }

            $combo_items = $combo_query->select('transaction_sell_lines.combo_modifier_ids', 'transaction_sell_lines.quantity')->get();

            $combo_modifier_qty = 0;
            $combo_modifier_total = 0;

            foreach ($combo_items as $combo_item) {
                $modifier_ids = explode(',', $combo_item->combo_modifier_ids);
                foreach ($modifier_ids as $modifier_id) {
                    $variation = \App\Models\Variation::find($modifier_id);
                    if ($variation) {
                        $combo_modifier_qty += $combo_item->quantity;
                        $combo_modifier_total += $variation->default_sell_price * $combo_item->quantity;
                    }
                }
            }

            if ($combo_modifier_qty > 0) {
                $found = false;
                foreach ($base_results as $result) {
                    $is_uncategorized = ($group_by == 'category' && empty($result->category_name)) ||
                                       ($group_by == 'brand' && empty($result->brand_name));

                    if ($is_uncategorized) {
                        $result->total_qty_sold += $combo_modifier_qty;
                        $result->subtotal += $combo_modifier_total;
                        $found = true;
                        break;
                    }
                }
                ...
                if (!$found) {
                    $new_row = (object)[
                        'category_name' => $group_by == 'category' ? null : '',
                        'brand_name' => $group_by == 'brand' ? null : '',
                        'current_stock' => 0,
                        'total_qty_sold' => $combo_modifier_qty,
                        'subtotal' => $combo_modifier_total
                    ];
                    $base_results->push($new_row);
                }
            }

            return Datatables::of($base_results)
    ...
    public function getStockValue(Request $request)
    {
        ...
        // If date filters are provided (for Profit Loss report), use date-filtered stock calculation
        if ($request->has('start_date') && $request->has('end_date')) {
            $start_date = $request->input('start_date');
            $end_date = $request->input('end_date');
            $location_id = $request->input('location_id');

            // Opening stock by sale price (day before start date)
            ...
            // Use the same method as Profit Loss report for date-filtered closing stock
            ...
            // For Stock report (no dates), use current stock
        ...
        return response()->json([
            'opening_stock_by_sp' => isset($opening_stock_by_sp) ? $opening_stock_by_sp : null,
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($activities)
                            ...
                            ->addColumn('subject_type', function($row) use($transaction_types) {
                                    ...
                                    } else if ($row->subject_type == 'App\Models\Transaction') {
                                        // Try to get type from subject, fallback to properties if deleted
                                        if (!empty($row->subject->type)) {
                                            $subject_type = isset($transaction_types[$row->subject->type]) ? $transaction_types[$row->subject->type] : '';
                                        } else {
                                            // Subject deleted, determine from description or properties
                                            if ($row->description == 'sell_deleted') {
                                                $subject_type = __('sale.sale');
                                            } else if ($row->description == 'so_deleted') {
                                                $subject_type = __('lang_v1.sales_order');
                                            } else {
                                                // Fallback to properties
                                                $attributes = $row->getExtraProperty('attributes');
                                                if (!empty($attributes['type']) && isset($transaction_types[$attributes['type']])) {
                                                    $subject_type = $transaction_types[$attributes['type']];
                                                }
                                            }
                                        }
                            ...
                            ->addColumn('note', function($row) use ($statuses, $shipping_statuses){
                                ...
                                // Get transaction details from subject or properties (if deleted)
                                ...
                                if (!empty($row->subject)) {
                                    // Subject exists
                                    $ref_no = $row->subject->ref_no ?? null;
                                    $invoice_no = $row->subject->invoice_no ?? null;
                                    $transaction_type = $row->subject->type ?? null;
                                } else if ($row->subject_type == 'App\Models\Transaction') {
                                    // Subject deleted, get from properties
                                    $attributes = $row->getExtraProperty('attributes');
                                    $transaction_type = $attributes['type'] ?? null;
                                }
                                
                                if (!empty($ref_no)) {
                                    $html .= __('purchase.ref_no') . ': ' . $ref_no . '<br>';
                                }
                                if (!empty($invoice_no)) {
                                    $html .= __('sale.invoice_no') . ': ' . $invoice_no . '<br>';
                                }
                                
                                if($row->subject_type == 'App\Models\Transaction' && !empty($row->subject) && in_array($transaction_type, ['sell', 'purchase'])) {
                                    $html .= view('sale_pos.partials.activity_row', ['activity' => $row, 'statuses' => $statuses, 'shipping_statuses' => $shipping_statuses])->render();
                                } else {
                                    // Show complete details for deleted transactions
                                    if ($row->description == 'sell_deleted' || $row->description == 'so_deleted') {
                                        $html .= '<span class="label label-danger">' . __('lang_v1.deleted') . '</span><br><br>';
                                        
                                        // Get complete invoice details from properties
                                        $customer_name = $row->getExtraProperty('customer_name');
                                        $transaction_date = $row->getExtraProperty('transaction_date');
                                        $location = $row->getExtraProperty('location');
                                        $final_total = $row->getExtraProperty('final_total');
                                        $payment_status = $row->getExtraProperty('payment_status');
                                        $status = $row->getExtraProperty('status');
                                        $items = $row->getExtraProperty('items');
                                        $payments = $row->getExtraProperty('payments');
                                        
                                        // Display invoice header info
                                        if (!empty($customer_name)) {
                                            $html .= '<strong>' . __('contact.customer') . ':</strong> ' . $customer_name . '<br>';
                                        }
                                        if (!empty($transaction_date)) {
                                            $html .= '<strong>' . __('messages.date') . ':</strong> ' . $transaction_date . '<br>';
                                        }
                                        if (!empty($location)) {
                                            $html .= '<strong>' . __('purchase.business_location') . ':</strong> ' . $location . '<br>';
                                        }
                                        if (!empty($status)) {
                                            $html .= '<strong>' . __('sale.status') . ':</strong> ' . ucfirst($status) . '<br>';
                                        }
                                        if (!empty($payment_status)) {
                                            $html .= '<strong>' . __('purchase.payment_status') . ':</strong> ' . ucfirst($payment_status) . '<br>';
                                        }
                                        if (!empty($final_total)) {
                                            $html .= '<strong>' . __('sale.total_amount') . ':</strong> ' . number_format($final_total, 2) . '<br>';
                                        }
                                        
                                        // Display items if available
                                        if (!empty($items) && is_array($items) && count($items) > 0) {
                                            $html .= '<br><strong>' . __('sale.products') . ':</strong><br>';
                                            foreach ($items as $item) {
                                                $html .= '• ' . ($item['product'] ?? '') . ' ' . ($item['variation'] ?? '') . ' - Qty: ' . ($item['quantity'] ?? 0) . ' @ ' . number_format($item['unit_price'] ?? 0, 2) . '<br>';
                                            }
                                        }
                                        
                                        // Display payments if available
                                        if (!empty($payments) && is_array($payments) && count($payments) > 0) {
                                            $html .= '<br><strong>' . __('sale.payments') . ':</strong><br>';
                                            foreach ($payments as $payment) {
                                                $html .= '• ' . ucfirst($payment['method'] ?? '') . ': ' . number_format($payment['amount'] ?? 0, 2) . '<br>';
                                            }
                                        }
                                        
                                    } else if ($row->subject_type == 'App\Models\Transaction' && empty($row->subject)) {
                                        // Fallback for old deleted transactions with limited data
                                        $attributes = $row->getExtraProperty('attributes');
                                        if (!empty($attributes)) {
                                            $html .= '<span class="label label-danger">' . __('lang_v1.deleted') . '</span><br>';
                                            if (!empty($attributes['status'])) {
                                                $html .= __('sale.status') . ': ' . ucfirst($attributes['status']) . '<br>';
                                            }
                                            if (!empty($attributes['payment_status'])) {
                                                $html .= __('purchase.payment_status') . ': ' . ucfirst($attributes['payment_status']) . '<br>';
                                            }
                                            if (!empty($attributes['final_total'])) {
                                                $html .= __('sale.total_amount') . ': ' . number_format($attributes['final_total'], 2) . '<br>';
                                            }
                                        }
                                    }
                            ...
                            ->editColumn('description', function($row) {
                                ...
                                else if($row->description == 'sell_deleted'){
                                    $desc .= '<span class="logout">' . __('lang_v1.' . $row->description) . '</span>';
                                } else {
                                    $desc .= '<span class="login">' . __('lang_v1.' . $row->description) . '</span>';
                                }
    ...
    public function gstSalesReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $base_results = $query->whereNull('transaction_sell_lines.parent_sell_line_id')->get();
            $expanded_results = collect();

            foreach ($base_results as $row) {
                $expanded_results->push($row);

                $combo_items = TransactionSellLine::where('parent_sell_line_id', $row->sell_line_id)
                    ->where('children_type', 'combo')
                    ->get();

                foreach ($combo_items as $combo_item) {
                    if (!empty($combo_item->combo_modifier_ids)) {
                        $modifier_ids = explode(',', $combo_item->combo_modifier_ids);
                        foreach ($modifier_ids as $modifier_id) {
                            $modifier_variation = \App\Models\Variation::with(['product.category', 'product.unit'])->find($modifier_id);
                            if ($modifier_variation && $modifier_variation->product) {
                                $modifier_row = (object)[
                                    'customer' => $row->customer,
                                    'supplier_business_name' => $row->supplier_business_name,
                                    'contact_id' => $row->contact_id,
                                    'tax_number' => $row->tax_number,
                                    'short_code' => $modifier_variation->product->category ? $modifier_variation->product->category->short_code : null,
                                    'transaction_id' => $row->transaction_id,
                                    'invoice_no' => $row->invoice_no,
                                    'transaction_date' => $row->transaction_date,
                                    'unit_price' => $modifier_variation->default_sell_price,
                                    'unit_price_after_discount' => $modifier_variation->default_sell_price,
                                    'sell_qty' => $combo_item->quantity,
                                    'discount_type' => null,
                                    'discount_amount' => 0,
                                    'item_tax' => 0,
                                    'tax_percent' => 0,
                                    'is_tax_group' => 0,
                                    'tax_id' => null,
                                    'unit' => $modifier_variation->product->unit ? $modifier_variation->product->unit->short_name : '',
                                    'parent_sell_line_id' => null, // Set to null so it's counted in totals
                                    'line_total' => $modifier_variation->default_sell_price * $combo_item->quantity,
                                    'children_type' => 'combo_modifier'
                                ];
                                $expanded_results->push($modifier_row);
                            }
                        }
                    }
                }
                $regular_modifiers = TransactionSellLine::where('parent_sell_line_id', $row->sell_line_id)
                    ->where('children_type', 'modifier')
                    ->with(['product.category', 'product.unit'])
                    ->get();

                foreach ($regular_modifiers as $modifier_item) {
                    if ($modifier_item->product) {
                        $modifier_row = (object)[
                            'customer' => $row->customer,
                            'supplier_business_name' => $row->supplier_business_name,
                            'contact_id' => $row->contact_id,
                            'tax_number' => $row->tax_number,
                            'short_code' => $modifier_item->product->category ? $modifier_item->product->category->short_code : null,
                            'transaction_id' => $row->transaction_id,
                            'invoice_no' => $row->invoice_no,
                            'transaction_date' => $row->transaction_date,
                            'unit_price' => $modifier_item->unit_price_before_discount,
                            'unit_price_after_discount' => $modifier_item->unit_price,
                            'sell_qty' => $modifier_item->quantity - $modifier_item->quantity_returned,
                            'discount_type' => $modifier_item->line_discount_type,
                            'discount_amount' => $modifier_item->line_discount_amount,
                            'item_tax' => $modifier_item->item_tax,
                            'tax_percent' => 0,
                            'is_tax_group' => 0,
                            'tax_id' => $modifier_item->tax_id,
                            'unit' => $modifier_item->product->unit ? $modifier_item->product->unit->short_name : '',
                            'parent_sell_line_id' => null, 
                            'line_total' => ($modifier_item->quantity - $modifier_item->quantity_returned) * $modifier_item->unit_price_inc_tax,
            ...
            $datatable = Datatables::of($expanded_results);
