...
class OrderController extends Controller
{
    ...
    public function markAsServed($id)
    {
        ...
        try {
            ...
            //  kitchen update event for real-time KDS updates
            event(new \App\Events\KitchenOrderUpdated(
                $id,
                null, // No specific item (whole order)
                'served',
                $business_id
            ));
            
            // Update cache timestamp for SSE fallback
            \Cache::put('kitchen_last_updated_' . $business_id, now(), 3600);
    ...
    public function markLineOrderAsServed($id)
    {
        try {
            ...
            if (!empty($sell_line)) {
                ...
                // Fire kitchen update event for real-time KDS updates
                event(new \App\Events\KitchenOrderUpdated(
                    $sell_line->transaction_id,
                    $sell_line->id,
                    'served',
                    $business_id
                ));
