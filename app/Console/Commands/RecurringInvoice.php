    ...
    public function handle()
    {
        try {
            ...
            foreach ($transactions as $transaction) {
                $tz = $transaction->business->time_zone ?: config('app.timezone');
                ...
                try {
                    ...
                    // (next_due_date check)
                    $last_dt = $no_of_recurring_invoice_generated > 0 ? $transaction->recurring_invoices->max('transaction_date') : $transaction->transaction_date;

                    $today = \Carbon\CarbonImmutable::today($tz);
                    $base = \Carbon\CarbonImmutable::parse($last_dt, $tz)->startOfDay();

                    // If first run and a repeat_on day is set for monthly, align base to that day in the same month
                    if ($transaction->recur_interval_type === 'months'
                        && !empty($transaction->subscription_repeat_on)
                        && $no_of_recurring_invoice_generated == 0) {
                        $repeatOn = (int) $transaction->subscription_repeat_on;
                        $base = $base->setDate($base->year, $base->month, min($repeatOn, $base->daysInMonth));
                    }

                    $next_due = null;
                    if ($transaction->recur_interval_type === 'days') {
                        $next_due = $base->addDays((int)$transaction->recur_interval);
                    } elseif ($transaction->recur_interval_type === 'months') {
                        $next_due = $base->addMonthsNoOverflow((int)$transaction->recur_interval);
                    } elseif ($transaction->recur_interval_type === 'years') {
                        $next_due = $base->addYears((int)$transaction->recur_interval)->startOfDay();
                    }

                    if ($next_due && $today->lt($next_due)) {
                        continue;
                    }

                    $today_str = $today->toDateString();
                    $already_today = Transaction::where('recur_parent_id', $transaction->id)
                        ->whereDate('transaction_date', $today_str)
                        ->exists();
                    if ($already_today) {
                        continue;
                    }
