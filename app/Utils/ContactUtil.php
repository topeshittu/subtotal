...
class ContactUtil extends Util
{
    ...
    public function getContactInfo($business_id, $contact_id)
    {
        $contact = Contact::where('contacts.id', $contact_id)
            ->where('contacts.business_id', $business_id)
            ->select(
                DB::raw("(SELECT SUM(t1.final_total) FROM transactions AS t1 WHERE t1.contact_id = contacts.id AND t1.type = 'purchase') AS total_purchase"),
                DB::raw("(SELECT SUM(t2.final_total) FROM transactions AS t2 WHERE t2.contact_id = contacts.id AND t2.type = 'sell' AND t2.status = 'final') AS total_invoice"),
                DB::raw("(SELECT SUM(t3.final_total) FROM transactions AS t3 WHERE t3.contact_id = contacts.id AND t3.type = 'sell_return') AS total_sell_return"),
                DB::raw("(SELECT SUM(t4.final_total) FROM transactions AS t4 WHERE t4.contact_id = contacts.id AND t4.type = 'purchase_return') AS total_purchase_return"),
                DB::raw("(SELECT SUM(tp1.amount) FROM transactions AS t5 LEFT JOIN transaction_payments AS tp1 ON t5.id = tp1.transaction_id WHERE t5.contact_id = contacts.id AND t5.type = 'purchase') AS purchase_paid"),
                DB::raw("(SELECT SUM(CASE WHEN tp2.is_return = 1 THEN -tp2.amount ELSE tp2.amount END) FROM transactions AS t6 LEFT JOIN transaction_payments AS tp2 ON t6.id = tp2.transaction_id WHERE t6.contact_id = contacts.id AND t6.type = 'sell'  AND t6.status = 'final') AS invoice_received"),
                DB::raw("(SELECT SUM(tp3.amount) FROM transactions AS t7 LEFT JOIN transaction_payments AS tp3 ON t7.id = tp3.transaction_id WHERE t7.contact_id = contacts.id AND t7.type = 'sell_return') AS sell_return_paid"),
                DB::raw("(SELECT SUM(t8.final_total) FROM transactions AS t8 WHERE t8.contact_id = contacts.id AND t8.type = 'opening_balance') AS opening_balance"),
                DB::raw("(SELECT SUM(tp4.amount) FROM transactions AS t9 LEFT JOIN transaction_payments AS tp4 ON t9.id = tp4.transaction_id WHERE t9.contact_id = contacts.id AND t9.type = 'opening_balance') AS opening_balance_paid"),
