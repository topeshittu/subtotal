...
class TransactionUtil extends Util
{
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        // Use input final_total when provided (includes shipping, packing, additional expenses, round off); fallback to calculated invoice total
        $final_total = isset($input['final_total']) ? ($uf_data ? $this->num_uf($input['final_total']) : $input['final_total'])
                      : (isset($invoice_total['final_total']) ? $invoice_total['final_total'] : 0);
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                if ($this->isModuleEnabled('modifiers')) {
                    ...
                    $existing_modifier_lines = TransactionSellLine::where('parent_sell_line_id', $product['transaction_sell_lines_id'])
                        ->where('children_type', 'modifier')
                        ->pluck('id')
                        ->toArray();
                    $kept_modifier_line_ids = [];
                    ...
                    if (!empty($product['modifier'])) {
                        foreach ($product['modifier'] as $key => $value) {
                            if (!empty($product['modifier_sell_line_id'][$key])) {
                                ...
                                $kept_modifier_line_ids[] = $product['modifier_sell_line_id'][$key];
                    ...
                    $modifier_lines_to_remove = array_diff($existing_modifier_lines, $kept_modifier_line_ids);
                    if (!empty($modifier_lines_to_remove)) {
                ...
                if (!empty($product['tax_id'])) {
                    $tax_rate_details = \App\Models\TaxRate::find($product['tax_id']);
                }
                $tax_rate_decimal = $tax_rate_details ? ($tax_rate_details->amount / 100) : 0;
                ...
                if (!empty($is_zatca_enabled)) {
                    $line_total_inc_tax = $uf_unit_price_inc_tax * ($uf_quantity * $multiplier);
                    $line_total_exc_tax = $line_total_inc_tax / (1 + $Main_Tax_Rate);
                    $total_item_tax = $line_total_inc_tax - $line_total_exc_tax;
    ...
    private function __makeLinesForComboProduct($combo_items, $parent_sell_line)
    {
        ...
        foreach ($combo_items as $value) {
            ...
            $modifier_ids = isset($value['single_modifier_ids']) ? $value['single_modifier_ids'] : (isset($value['modifier_ids']) ? $value['modifier_ids'] : null);
            ...
            $combo_lines[] = new TransactionSellLine([
                ...
                'children_type' => 'combo',
                'combo_modifier_ids' => $modifier_ids
            ]);
        }
    ...
    public function deleteSellLines($transaction_line_ids, $location_id, $adjust_qty = true)
    {
        if (!empty($transaction_line_ids)) {
            ...
            // Delete each sell line individually to trigger model events for modification logging
            foreach ($sell_lines as $line) {
                $line->delete();
            }
        }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_reward_point == 1) {
            ...
            $total_rp_value = $customer->total_rp * $business_details->redeem_amount_per_unit_rp;
            $output['customer_total_rp_value'] = $this->num_f($total_rp_value, true, $business_details);
        }
        ...
        if (blank($il->date_time_format)) {
            ...
            // $tz = $business_details->time_zone ?: (session('business.time_zone') ?: config('app.timezone'));
            // $output['invoice_date'] = Carbon::createFromFormat('Y-m-d H:i:s', $transaction->transaction_date, $tz)
        ...
        if (!empty($il->common_settings['show_due_date']) && $transaction->payment_status != 'paid') {
            ...
            if (!empty($due_date)) {
                if (blank($il->date_time_format)) {
                    ...
                    // $tz = $business_details->time_zone ?: (session('business.time_zone') ?: config('app.timezone'));
                    // $output['due_date'] = Carbon::createFromFormat('Y-m-d H:i:s', $due_date->toDateTimeString(), $tz)
                    //     ->format($il->date_time_format);
                }
            }
        }
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($details['lines'] as $line) {
                ...
                //Total line tax
                $output['total_line_tax'] = $this->num_f($total_line_taxes, $show_currency, $business_details);
                //zatca code
                if (!empty($is_zatca_enabled)) {
            ...
            $output['subtotal_exc_tax'] = $this->num_f($subtotal_exc_tax + $total_line_discount, $show_currency, $business_details);
           // $this->num_f($paid_amount, $show_currency, $business_details);

            $output['total_line_discount'] = !empty($total_line_discount) ? $this->num_f($total_line_discount, true, $business_details) : 0;
            $output['total_line_discount_uf'] = !empty($total_line_discount) ? $total_line_discount : 0;
        ...
        $is_currencyexchange_enabled = app(\App\Utils\ModuleUtil::class)->isModuleInstalled('CurrencyExchangeRate');
        if (!empty( $is_currencyexchange_enabled)) {
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            //Subtotal Exc Tax Exc discount
            $line_array['line_total_exc_tax_exc_discount'] += $line->unit_price_before_discount * $line->quantity;
            ...
            if ($il->show_image == 1) {
                ...
                if (count($media)) {
                    ...
                    $imageUrl = $first_img->display_url ?? null;
                } else {
                ...
                foreach ($combo_variations as $combo_variation) {
                    ...
                    if ($combo_variation_details) {
                        ...
                        $combo_line_array = [
                            ...
                            'warranty' => $combo_product->warranty ? $combo_product->warranty->name : '',
    ...
    public function getOpeningClosingStock(
        ...
        array  $filters        = [],
        ?string $period_end_date = null
    ): float {
        // 1) Build your date filter (opening vs closing)
        // For opening stock: include opening_stock transactions dated up to the period end date
        // This handles cases where opening stock is added during the reporting period
        if ($is_opening && $period_end_date) {
            $dateFilter = function($q) use ($date, $period_end_date) {
                $q->where(function($subQuery) use ($date, $period_end_date) {
                    $subQuery->where(function($q2) use ($date) {
                        // Regular transactions: dated on or before opening date
                        $q2->whereDate('t.transaction_date', '<=', $date)
                           ->where('t.type', '!=', 'opening_stock');
                    })->orWhere(function($q3) use ($period_end_date) {
                        // Opening stock transactions: dated on or before period end
                        $q3->where('t.type', 'opening_stock')
                           ->whereDate('t.transaction_date', '<=', $period_end_date);
                    });
                });
            };
        } else {
        ...
        $final = (float) ($purchaseValue + $salesValue + $adjustValue);
        return $final;
    }
    /**
    ...
    public function isCustomerCreditLimitExeeded(
        ...
        //If draft, running order, or suspended order ignore credit limit check
        if ($input['status'] == 'draft' || 
            (isset($input['type']) && $input['type'] == 'sales_order') ||
            (isset($input['is_running_order']) && $input['is_running_order'] == 1) ||
            (isset($input['is_suspend']) && $input['is_suspend'] == 1) ||
            !isset($input['final_total'])) {
            return false;
        }
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if (!empty($transaction)) {
            // Load relationships for complete details
            $transaction->load(['contact', 'location', 'sell_lines.product', 'sell_lines.variations', 'payment_lines']);
            
            // Prepare complete invoice details for activity log
            $log_properities = [
                'id' => $transaction->id,
                'invoice_no' => $transaction->invoice_no,
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        //Get Opening stock
        $opening_stock = $this->getOpeningClosingStock($business_id, $day_before_start_date, $location_id, true, false, $filters, $end_date);
