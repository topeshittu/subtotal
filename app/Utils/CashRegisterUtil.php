...
{
    $product_details_raw = Transaction::where('transactions.created_by', $user_id)
        ...
        ->groupBy('v.id', 'TSL.children_type', 'TSL.id', 'TSL.combo_modifier_ids')
        ->select(
            DB::raw('CASE 
                WHEN TSL.children_type = "modifier" 
    ...
    foreach ($product_details_raw as $row) {
        if ($row->children_type === 'combo' && !empty($row->combo_modifier_ids)) {
            $ids = explode(',', $row->combo_modifier_ids);

            $modifiers = Variation::whereIn('id', $ids)
                ->with(['product.brand', 'product_variation'])
                ->get();

            foreach ($modifiers as $mod) {
                $product_details->push((object)[
                    'product_name' => $mod->product->name . ' - ' . $mod->name,
                    'product_type' => $mod->product->type,
                    'type' => $mod->product->type,
                    'variation_name' => $mod->name,
                    'product_variation_name' => $mod->product_variation->name,
                    'sku' => $mod->sub_sku,
                    'children_type' => 'combo_item',
                    'brand_name' => $mod->product->brand ? $mod->product->brand->name : null,
                    'total_quantity' => $row->total_quantity, 
                    'total_amount' => $mod->default_sell_price * $row->total_quantity
                ]);
            }
        } elseif ($row->children_type !== 'combo') {
    ...
    foreach ($product_details as $row) {
        ...
        if (!$grouped->has($key)) {
            $grouped[$key] = (object)[
                ...
                'sku' => $row->sku,
                'children_type' => $row->children_type,
        ...
        $grouped[$key]->total_quantity += $row->total_quantity;
        $grouped[$key]->total_amount += $row->total_amount;
    }

    $product_details = $grouped->values();

    $product_details_by_brand = $product_details->groupBy(function ($item) {
        return $item->brand_name ?? null;
    })->map(function ($group, $brand_name) {
        return (object) [
