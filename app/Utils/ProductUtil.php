...
class ProductUtil extends Util
{
    ...
    public function calculateInvoiceTotal($products, $tax_id, $discount = null, $uf_number = true)
    {
        ...
        //Sub Total - CORRECTED: For tax-inclusive products, use tax-inclusive line totals
        foreach ($products as $key => $product) {
            if (isset($product['children_type']) && $product['children_type'] == 'combo') {
                $combo_total = $transaction->sell_lines()
                    ->where(function($q) {
                        $q->whereNull('children_type')
                          ->orWhere('children_type', '!=', 'combo');
                    })
                    ->sum(\DB::raw('unit_price_inc_tax * quantity'));

            $combo_items = $transaction->sell_lines()
                    ->where('children_type', 'combo')
                    ->whereNotNull('combo_modifier_ids')
                    ->where('combo_modifier_ids', '!=', '')
                    ->get();

            foreach ($combo_items as $combo_item) {
                    $modifier_ids = explode(',', $combo_item->combo_modifier_ids);
                    $modifier_prices_sum = \DB::table('variations')
                        ->whereIn('id', $modifier_ids)
                        ->sum('default_sell_price');
                   $output['total_before_tax'] += $combo_total + $modifier_prices_sum * $combo_item->quantity;
                }
            }
            ...
            // For tax-inclusive/inline tax products, total_before_tax = sum of tax-inclusive line totals
            ...
            // Add combo modifiers by IDs (from combo children) if present
            if (!empty($product['combo']) && is_array($product['combo'])) {
                $parent_qty = $quantity; // already uf'ed above
                foreach ($product['combo'] as $child) {
                    $child_qty = isset($child['quantity']) ? ($uf_number ? $this->num_uf($child['quantity']) : $child['quantity']) : 0;

                    // Combine modifier IDs from possible fields
                    $ids = [];
                    foreach (['modifier_ids', 'single_modifier_ids'] as $field) {
                        if (!empty($child[$field])) {
                            $parts = array_filter(array_map('trim', explode(',', $child[$field])));
                            if (!empty($parts)) {
                    ...
                    if (!empty($ids)) {
                        ...
                        $modifier_prices_sum = Variation::whereIn('id', $ids)->sum('default_sell_price');
                        // Multiply by child quantity per parent and parent quantity
                        $output['total_before_tax'] += ($modifier_prices_sum * $child_qty * $parent_qty);
                    }
                }
            }
        }
