...
class Util
{
    ...
    public function uf_date($date, $time = false)
    {
        ...
        $sourceTz = session('business.time_zone') ?: config('app.timezone');
        return ! empty($date_format)
            ? \Carbon::createFromFormat($date_format, $date, $sourceTz)->format($mysql_format)
            : null;
    }
    ...
    public function format_date($date, $show_time = false, $business_details = null)
    {
        ...
        $targetTz = (! empty($business_details) && ! empty($business_details->time_zone))
            ? $business_details->time_zone
            : (session('business.time_zone') ?: config('app.timezone'));
        return \Carbon::parse($date, $targetTz)->format($format);
    }
    ...
    public function updateEditedSellLineCombo($lines, $location_id, $adjust_stock = true)
    {
        ...
        foreach ($lines as $key => $line) {
            ...
            if (isset($line['modifier_ids']) || isset($line['single_modifier_ids'])) {
                $modifier_ids = isset($line['single_modifier_ids']) ? $line['single_modifier_ids'] : (isset($line['modifier_ids']) ? $line['modifier_ids'] : null);
                $prev_line->combo_modifier_ids = $modifier_ids;
                $prev_line->save();
            }
    ...
    public function giveLocationPermissions($user, $request)
    {
     ...
     public function getSubUnitsApi($business_id, $unit_id, $return_main_unit_if_empty = false, $product_id = null)
    {
        if (is_null($unit_id)) {
            return [];
        }
