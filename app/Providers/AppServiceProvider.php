...
class AppServiceProvider extends ServiceProvider
{
    ...
    {
        $isDemoReset = DemoResetHelper::isResetting();
            ...
            function ($view) {
                ...
                if (isAppInstalled()) {
                    try {
                        ...
                        // Tables might not exist during reset
                        return;
                    }
        ...
        Blade::directive('format_date', function ($date) {
            if (!empty($date)) {
                return "Carbon\\Carbon::parse($date, session('business.time_zone') ?: config('app.timezone'))->format(session('business.date_format'))";
            } else {
                return null;
            }
        });
        ...
        Blade::directive('format_time', function ($date) {
            if (!empty($date)) {
                ...
                return "Carbon\\Carbon::parse($date, session('business.time_zone') ?: config('app.timezone'))->format('$time_format')";
            } else {
                return null;
            }
        });
        ...
        Blade::directive('format_datetime', function ($date) {
            if (! empty($date)) {
                ...
                return "Carbon\\Carbon::parse($date, session('business.time_zone') ?: config('app.timezone'))->format(session('business.date_format') . ' ' . '$time_format')";
            } else {
                return null;
            }
        });
        ...
        View::composer('menus.partials.*', function($view) {
            if (DemoResetHelper::isResetting()) {
                $view->with('menus', []);
                return;
            }
            try {
                $view->with('menus', MenuService::sidebar(Auth::id() ?: 0));
            } catch (\Exception $e) {
                $view->with('menus', []);
            }
        });
