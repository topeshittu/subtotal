...
use Modules\Desktopapp\Notifications\NewPassword;
...
class UserController extends ApiController
{
    ...
    public function index()
    {
        ...
        if (!empty(request()->service_staff) && request()->service_staff == 1) {
            ...
            $users = User::where('business_id', $business_id)
                        ...
                        ->get();
            //$permissions = [];
            $users = $users->map(function ($item) use ($business_id) {
                return collect([
                    'id' => $item->id,
                    ...
                    'essentials_department_id' => $item->essentials_department_id,
                    'essentials_designation_id' => $item->essentials_designation_id,
                    'essentials_salary' => $item->essentials_salary,
                    'essentials_pay_period' => $item->essentials_pay_period,
                    'essentials_pay_cycle' => $item->essentials_pay_cycle,
                    'allow_login' => $item->allow_login,
                    'status' => $item->status,
                    'crm_contact_id' => $item->crm_contact_id,
                    'is_cmmsn_agnt' => $item->is_commsn_agnt,
                    'cmmsn_percent' => $item->cmmsn_percent,
                    'selected_contacts' => $item->selected_contacts,
                    'dob' => $item->dob,
                    'gender' => $item->gender,
                    ...
                    'social_media_1' => $item->social_media_1,
                    'social_media_2' => $item->social_media_2,
                    'permanent_address' => $item->permanent_address,
                    'current_address' => $item->current_address,
                    'guardian_name' => $item->guardian_name,
                    'custom_field_1' => $item->custom_field_1,
                    'custom_field_2' => $item->custom_field_2,
                    'custom_field_3' => $item->custom_field_3,
                    'custom_field_4' => $item->custom_field_4,
                    'bank_details' => $item->bank_details,
                    'id_proof_name' => $item->id_proof_name,
                    'id_proof_number' => $item->id_proof_number,
                    'location_id' => $item->location_id,
                    'crm_department' => $item->crm_department,
                    'crm_designation' => $item->crm_designation,
                    'locationIds' => User::find($item->id)->permitted_locations($business_id, 'api'),
                    'permissions' => User::find($item->id)->getAllPermissions()->map(function ($permission){
    ...
    public function loggedin()
    {
        ...
        if($user->business->kyc_verification == 2){
            $user->propay = true;
        }else{
            $user->propay = false;
        }
