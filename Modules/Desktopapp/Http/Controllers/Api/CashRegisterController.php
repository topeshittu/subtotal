...
class CashRegisterController extends ApiController
{
     ...
     * @response {
        ...
        "links": {
            "first": "http://local.pos.com/desktopapp/api/cash-register?page=1",
            "last": null,
            "prev": null,
            "next": null
        },
        "meta": {
            "current_page": 1,
            "from": 1,
            "path": "http://local.pos.com/desktopapp/api/cash-register",
            "per_page": 10,
            "to": 2
        }
    }
    ...
    */
    public function store(Request $request)
    {
        try {
            ...
            'closing_note', 'closing_amount', 'total_card_slips', 'total_cheques', 'serverId', 'user_name']);

            $getUser = User::where('username', $register_data['user_name']);
            if ($getUser->exists()) {
                $user = $getUser->first();
            } else {
                $user = Auth::user();
            ...
            $register = CashRegister::updateOrCreate([
                'unique_register_id' => $register_data['serverId']
            ],[
                'status' => $register_data['status'],
                'location_id' => $register_data['location_id'],
                'closing_note' => $register_data['closing_note'],
                'closing_amount' => $register_data['closing_amount'],
                'total_card_slips' => $register_data['total_card_slips'],
                'total_cheques' => $register_data['total_cheques'],
                'created_at' => \Carbon::parse($request->input('created_at'))->format('Y-m-d H:i:s'),
                'closed_at' => \Carbon::parse($request->input('closed_at'))->format('Y-m-d H:i:s'),
            ...
            foreach ($sells as $sell) {
                foreach ($sell->payment_lines as $payment) {
                    $cash_register_payments[] = new CashRegisterTransaction([
                        ...
                        'type' => 'credit',
                        'transaction_type' => $sell->type,
                        'transaction_id' => $sell->id
                    ]);
                }

                //Update transactions_created_at
                $sell->update([
                    'created_at' => \Carbon::parse($request->input('closed_at'))->subSeconds(10)->format('Y-m-d H:i:s')
                ]);
            }
