            ...
            return DataTables::of($documents)
                ...
                ->editColumn(
                    ...
                    @if($type == "document")
                        ...
                        @php
                            try {
                                $size = \Illuminate\Support\Facades\Storage::disk((app(\App\Services\AppSettingsService::class))->storage_default_disk() ?: config("filesystems.default"))->size("documents/" . $name);
                            } catch (Exception $e) {
                                try {
                                    $size = filesize(public_path("uploads/documents/" . $name));
                                } catch (Exception $e2) {
                                    $size = null;
                                }
                            }
