            ...
            return Datatables::of($todos)
                    ...
                    function ($row) {
                        $html = '<div class="btn-group">
                            <button type="button" class="dropdown-toggle btn-xs" data-toggle="dropdown" aria-expanded="false">
                                <img src="' . asset('img/icons/item.svg') . '" alt="">                            </button>
                ...
                ->editColumn('task', function ($row) use ($priorities) {
                    $html = '<a href="'.action([\Modules\Essentials\Http\Controllers\ToDoController::class, 'show'], [$row->id]).'" >'.$row->task.'</a> <br>
                        <a data-href="'.action([\Modules\Essentials\Http\Controllers\ToDoController::class, 'viewSharedDocs'], [$row->id]).'" class="btn custom-sm-button-docs btn-primary btn-xs  view-shared-docs">'.__('essentials::lang.docs').'</a>';
