	 ...
	 <div class="col-sm-6">
		<div class="form-group">
			...
			<div class="toggle-wrapper d-flex gap-2 mt-4">
				<label class="switch" for="{{ $index === '' ? 'allow_login' : 'allow_login_'.$index }}">
					{!! Form::checkbox($index === '' ? 'allow_login' : "contact_persons[$index][allow_login]", 1, false, ['id' => $index === '' ? 'allow_login' : "allow_login_{$index}", 'class' => 'allow_login', 'data-loginDiv' => "loginDiv{$index}"]) !!}
