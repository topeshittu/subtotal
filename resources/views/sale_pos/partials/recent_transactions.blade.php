...
@if(!empty($transactions))
	<table class="table table-slim no-border">
		@foreach ($transactions as $transaction)
			<tr class="cursor-pointer" title="Customer: @if(!empty($transaction->contact->name))({{ optional($transaction->contact)->name }})@elseif(empty($transaction->contact->name))({{ optional($transaction->contact)->supplier_business_name }})@endif 
@if(!empty($transaction->contact->mobile) && $transaction->contact->is_default == 0)Mobile: {{$transaction->contact->mobile}} @endif" >
				<td>
					{{ $loop->iteration}}.
				</td>
				<td>
				...
				@elseif(empty($transaction->contact->name))
