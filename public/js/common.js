...
$(document).ready(function() {
   ...
   var icon = "{{ asset('img/icons') }}";
    ...
    {
        extend: 'csv',
        text: '<i class="fa fa-file-csv" aria-hidden="true"></i> <span class="btn-text"> ' + LANG.export_to_csv + '</span>',
    ...
    {
        extend: 'excel',
        text: '<i class="fa fa-file-excel" aria-hidden="true"></i><span class="btn-text"> ' + LANG.export_to_excel + '</span>',
        className: 'btn btn-sm ',
        exportOptions: {
            columns: ':visible',
        },
    ...
    {
        extend: 'print',
        text: '<i class="fa fa-print" aria-hidden="true"></i> <span class="btn-text"> ' + LANG.print + '</span>',
        className: 'btn btn-sm buttons-html5',
        exportOptions: {
            columns: ':visible',
            stripHtml: true,
        },
    ...
    {
        extend: 'colvis',
        text: '<i class="fa fa-columns" aria-hidden="true"></i> <span class="btn-text"> ' + LANG.col_vis + '</span>',
        className: 'btn btn-sm buttons-html5',
    },
];
...
var pdf_btn = {
    extend: 'pdf',
    text: '<i class="fa fa-file-pdf" aria-hidden="true"></i> <span class="btn-text"> ' + LANG.export_to_pdf + '</span>',
    className: 'btn btn-sm buttons-html5',
    exportOptions: {
        columns: ':visible',
    },
...
// Function to get the appropriate loader image URL
function getLoaderImageUrl() {
    if (isDarkModeEnabled()) {
        return icon_url + '/dark-loader.svg';
    } else {
        return icon_url + '/loader.svg';
    }
}
...
jQuery.extend($.fn.dataTable.defaults, {
    ...
    language: {
        ...
        processing: '<img src="' + getLoaderImageUrl() + '">',
...
// Update the loader image if dark mode changes dynamically
var observer = new MutationObserver(function() {
    var processingElem = document.querySelector('.dataTables_processing');
    if (processingElem) {
        processingElem.innerHTML = '<img src="' + getLoaderImageUrl() + '">';
    }
});
...
$(document).on('click', '#toggle-sidebar', function () {
    if ($('body').hasClass('sidebar-collapse')) {
        ...
        localStorage.setItem('bpos_sidebar_collapse', 'false');
    } else {
        $('body').addClass('sidebar-collapse');
        localStorage.setItem('bpos_sidebar_collapse', 'true');
    }
});
   var sidebar_toggle_button = $('#toggle-sidebar'); 
   var mobile_sidebar_toggle_button = $('#menuBtn'); 
   var close_btn = $('#close-btn'); 
   var sidebar = $('#main-sidebar');
   var main_content = $('main');
   var main_container = $('main .main-container');
   var body = $('body');
   function hide_sidebar() {
       body.addClass('sidebar-collapse');
       main_content.addClass('sidebar-hidden');
       sidebar_toggle_button.attr('aria-expanded', 'false');
   }
   
       function show_sidebar() {
       body.removeClass('sidebar-collapse');
       main_content.removeClass('sidebar-hidden');
       sidebar_toggle_button.attr('aria-expanded', 'true');
       }
   if (mobile_sidebar_toggle_button.length) {
       mobile_sidebar_toggle_button.on('click', function (event) {
   ...
   function check_window_width() {
    ...
    var sidebar_collapse = localStorage.getItem('bpos_sidebar_collapse') || 'false';
