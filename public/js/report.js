$(document).ready(function () {
    ...
    product_sell_report_with_purchase_table = $('table#product_sell_report_with_purchase_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[4, 'desc']], // This table has different columns, keep as is
        ajax: {
...
function get_stock_value() {
    ...
    // Only add date filters if profit_loss_date_range exists (Profit Loss page)
    if ($('#profit_loss_date_range').length > 0 && $('#profit_loss_date_range').data('daterangepicker')) {
        data.start_date = $('#profit_loss_date_range').data('daterangepicker').startDate.format('YYYY-MM-DD');
        data.end_date = $('#profit_loss_date_range').data('daterangepicker').endDate.format('YYYY-MM-DD');
    }
